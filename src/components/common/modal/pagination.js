...
const Pagination = ({ pageCount, handlePageClick, currentPage }) => {
  ...
  return (
    <BootstrapPagination className="justify-content-center mt-4">
      <BootstrapPagination.Prev onClick={() => handlePageChange(currentPage > 0 ? currentPage : 0)} disabled={currentPage === 0} />
      ...
      <BootstrapPagination.Next onClick={() => handlePageChange(currentPage < pageCount - 1 ? currentPage + 2 : pageCount)} disabled={currentPage === pageCount - 1} />
    </BootstrapPagination>
  );
};
